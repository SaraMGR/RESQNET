...
// Mapa para almacenar los dispositivos encontrados listos para conectar.
std::map<std::string, BLEAdvertisedDevice*> advertisedDevices; 

// Nuevo: Mapas para gestionar las conexiones activas.
// La clave es el nombre del nodo.
...
void notifyCallback(BLERemoteCharacteristic* pBLERemoteCharacteristic, uint8_t* pData, size_t length, bool isNotify) {
	...
	// Usamos .c_str() para imprimir la dirección sin errores de String vs std::string
	Serial.print(pBLERemoteCharacteristic->getRemoteService()->getClient()->getPeerAddress().toString().c_str());
	Serial.print("): ");
